...
import { useNavigate } from 'react-router-dom';
import NoDataFound from '../../../components/userComponents/NoDataComponent';
import DoctorListComponent from '../../../components/userComponents/DoctorListComponent';
...
function DoctorsList() {
  ...
  const search =(doctors)=>{
    return doctors?.docs?.filter((item)=>
  ...
  useLayoutEffect(() => {
    ...
    console.log('lkldkledkkklewdkledwemwdod');
    getRequest(`/user/department-doctors/${department}/${currentPage}`).then((res) => {
      console.log('depdoctors...................', res);
      setDoctors(res);
      setLoading(false);
  ...
  return (
    <>
    <h1 className=" text-2xl md:text-3xl lg:text-4xl font-heading text-gray mt-8 ml-4">
   Search
    </h1>
    ...
    <DoctorListComponent doctors={search(doctors)} currentPage={currentPage}  setCurrentPage={setCurrentPage}  totalPages={totalPages} />
