...
const ListOfPrescriptions = () => {
  ...
  useEffect(() => {
    const fetchPrescriptions = async () => {
      try {
        const prescriptionData = await getRequest(`/doctor/prescriptions/${userId}`)
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        console.log("122222222222222userIduserIduserId",userId)
        const userData = await getRequest(`/doctor/user/${userId}`)
        console.log("userData latest",userData);
        
        
        setUser(userData?.alluser?.[0]);
 ...
 const deleteHandler=async(prescriptionId)=>{
  try{
    console.log("prescriptionId",prescriptionId);
    await getRequest(`/doctor/deletePrescription/${prescriptionId}`).then((response)=>{
        ...
        <div className="flex flex-col col-span-3">
          <div className="flex flex-col space-y-4 p-6 text-gray-600">
            ...
            <div className="flex flex-row items-center justify-between text-sm">
              ...
              <Link  
                to={`/doctor/newprescription/${userId}`}
